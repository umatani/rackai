...
(define-reduction (==> --> :=<1>)
  ...
  [(ζ (Stxξ ph (and (Stx (Lst (? id? id_lam)
      ...
      κ₀
      (Σ̂ Σ₀ scpsₚ scpsᵤ))
   lambda? :=<1> (id=? ph id_lam 'lambda ξ Σ₀)
   #:when lambda?
  ...
  [(ζ (Stxξ ph (and (Stx (Lst (? id? id_quote) _) _ctx) stx) ξ)
      κ₀
      (Σ̂ Σ₀ scpsₚ scpsᵤ))
   quote? :=<1> (id=? ph id_quote 'quote ξ Σ₀)
   #:when quote?
  ...
  [(ζ (Stxξ ph (Stx (Lst (? id? id_syntax) stx) ctx) ξ)
      κ₀
      (Σ̂ Σ₀ scpsₚ scpsᵤ))
   syntax? :=<1> (id=? ph id_syntax 'syntax ξ Σ₀)
   #:when syntax?
  ...
  [(ζ (Stxξ ph (and (Stx (Lst (? id? id_ls)
      ...
      κ₀
      (Σ̂ Σ₀ scpsₚ scpsᵤ))
   let-syntax? :=<1> (id=? ph id_ls 'let-syntax ξ Σ₀)
   #:when let-syntax?
