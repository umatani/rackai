...
;; short-cuts
(define-syntax-rule (DFS . args)
  (default-font-size . args))
(define-syntax-rule (DS)
  (default-style))
(define (rm txt) (text txt 'roman (DFS)))
...
(define (prim s) (text s '(bold . modern) (DFS)))
(define (ast s) (text s '(bold caps . roman) (DFS)))
(define (trans s) (text s '(caps . roman) (DFS)))
(define (lit s) (text s (literal-style) (DFS)))
(define (mf s) (text s (metafunction-style) (metafunction-font-size)))
(define (atag s) (trans s))

(define transform (text "transform" (non-terminal-style) (DFS)))
...
(define (fix-subscript p)
  (let ([p (symbol->string p)])
    (hbl-append (text (substring p 0 1) (non-terminal-style) (DFS))
                (text (substring p 2) (non-terminal-subscript-style) (DFS)))))
...
(define (name)
  (text "name" (non-terminal-style) (DFS)))
(define (var)
  (text "var" (non-terminal-style) (DFS)))
...
(define (scps) (overline (text "scp" (non-terminal-style) (DFS))
                         #:delta 0.2))

(define (bullet) (text "•" 'default (DFS)))
  ...
  (parameterize
      ([literal-style 'modern]
       ...
       [where-make-prefix-pict (const
                                ((current-text) "    where  " (DS) (DFS)))]
       [where-combine (lambda (l r)
                        (define =-pict ((current-text) " = " (DS) (DFS)))
                        (hbl-append l =-pict r))])
    (with-atomic-rewriters
      (['nam name]
       ...
       ['Σ             (const (text "Σ" (DS) (DFS)))]
       ['Σ*            (const (hat (text "Σ" (DS) (DFS))))]
       ['maybe-scp     (const (hat #:delta 0.2
                                   (text "scp" (non-terminal-style) (DFS))))]
       ['δ             (const (text "δ"      (DS)     (DFS)))]
       ['kont          (const (text "κ"      'default (DFS)))]
       ['env           (const (text "\u03BE" (DS)     (DFS)))]
       ['....          (const (text "...."   'default (DFS)))]
       ['SPC           (const (text " "      'default (DFS)))]
       ['all-tprim     (const (text "tprim"     (non-terminal-style) (DFS)))]
       ['all-transform (const (text "transform" (non-terminal-style) (DFS)))]
       ['pre-ctx       (const (text "ctx"       (non-terminal-style) (DFS)))]
       ['pre-val       (const (text "val"       (non-terminal-style) (DFS)))]
       ...
       ['desc-ctx (const (hbl-append
                          (rm "a mapping from ")
                          (text "ph" (non-terminal-style) (DFS))
                          (rm " to ")
                          (scps)))]
          ...
          (lambda (lw)
            (cond
              ...
              [(and (pair? (lw-e lw))
               ...
               (define (s sub)
                 (hbl-append (scps)
                             (text sub (non-terminal-subscript-style) (DFS))))
               (define subset (rm " ⊆ "))
               (re-lw (vl-append
                       ((current-render-pict-adjust)
  ...
  (apply htl-append
         (add-between
          (for/list ([e (in-list es)])
            (if (pict? e)
                e
                (text e '(italic . roman) (DFS))))
          (text " " '(italic . roman) (DFS)))))
