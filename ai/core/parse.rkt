...
(define (parse stx Σ)
  ...
  (match stx
    ...
    [(GenStx `(,(? Id? (? (id=? 'lambda)))
     ...
     (for*/set ([var_list (in-set (build-alt-lists
                                   (for/list ([id (in-list stl_ids)])
                                     (for/list ([nam (in-set (resolve id Σ))])
                                       (Var nam)))))]
                [ast_body (parse stx_body Σ)])
     ...
     (let-values ([(stl_ids stl_rhs) (unzip stl_binds)])
       (for*/set
            ([var_list (in-set (build-alt-lists
                                (for/list ([id (in-list stl_ids)])
                                  (for/set ([nam (in-set (resolve id Σ))])
                                    (Var nam)))))]
             [rhs_list (in-set (build-alt-lists
                                (for/list ([stx (in-list stl_rhs)])
                                  (for/set ([ast (in-set (parse stx Σ))])
                                    ast))))]
             [ast_body (in-set (parse stx_body Σ))])
    ...
    [(GenStx (cons (? Id? (? (id=? '#%app)))
     ...
     (for*/set ([ast_fun  (in-set (parse stx_fun Σ))]
                [ast_args (in-set (parse* stl_args Σ))])
       (App ast_fun ast_args))]
    ; (if stx stx stx)
    [(GenStx `(,(? Id? (? (id=? 'if))) ,stx_test ,stx_then ,stx_else) _)
     (for*/set ([ast_test (in-set (parse stx_test Σ))]
                [ast_then (in-set (parse stx_then Σ))]
                [ast_else (in-set (parse stx_else Σ))])
       (If ast_test ast_then ast_else))]
    ; reference
    [(? Id? id)
     (for/set ([nam (in-set (resolve id Σ))]) (Var nam))]
    ; literal
  ...
  (match stl
    ...
    [(cons stx stl)
     (for*/set ([ast (in-set (parse stx Σ))]
       ...
       (cons ast asts))]
    [stx (for/set ([ast (in-set (parse stx Σ))])
           (list ast))]))
