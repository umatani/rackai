...
(require redex redex/parameter
         "common.rkt"
         slideshow/pict
         (rename-in (except-in "phases-machine.rkt"
                               stl->seq
                               update-ctx
                               at-phase)
                    [lookup-σ phases:lookup-σ]
                    [run phases:run]
                    [alloc-loc* phases:alloc-loc*])
...
(define-extended-language Lloc Lph
  ...
  [cont ::=
        •
        (App (ph maybe-scp env) val ... [] clo ... loc) ; updated
        (If [] clo clo loc)]
  [ser ::=
       (ph ast ρ maybe-scp env) ; updated
       (App (ph maybe-scp env) clo ...) ; updated
       (If clo clo clo)
       (Seq clo ...) ; new
       ]
  [state ::=
         (clo cont σ Σ*) ; updated
  ...
  [κ ::=
     •
     (STX ex? Σ* loc) ; updated
     ]
  [cfg ::=
       (stx∘ ex? κ σ Σ*) ; updated
       (in-eval state cfg)]
  )
...
(define-extended-metafunction* phases:lookup-σ Lloc
  lookup-σ : σ loc -> u)
...
(define-metafunction Lloc
  ...
  [(alloc-loc ((App (_ _ _) val ... clo_0 clo ...)
               cont (Heap number [loc u] ...) Σ*))
   (values ,(string->symbol (format "vapp:~a" (term number)))
           (Heap ,(add1 (term number)) [loc u] ...))]
  [(alloc-loc ((If clo_test clo_then clo_else)
               cont (Heap number [loc u] ...) Σ*))
...
(define-metafunction/extension phases:alloc-loc* Lloc
...
(define -->c
  (reduction-relation
   ...
   ;; if
   (--> (name s ((If ser_test clo_then clo_else) cont σ Σ*))
...
(define ==>c
  (reduction-relation
   Lloc #:domain cfg #:arrow ==> ;; cfg = (stx∘ ex? κ σ Σ*)
