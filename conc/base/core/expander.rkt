...
(define-reduction (==> --> :=<1>)
  #:within-signatures [(only terms^
                             ...
                             TVar% Stxξ% Hole% κ% InEval% ζ%)
                       (only terms-extra^
                             lst->list snoc val? stx? id? proper-stl?)
                       (only syntax^
                             empty-ctx zip unzip in-hole add flip)
                       (only env^
                             init-env)
                       (only store^
                             init-store)
                       ...
                       (only parser^
                             parse)]
  #:do [(use-terms App Atom Sym Stx List Null Pair AstEnv TVar Stxξ Hole
                   κ InEval ζ)
