...
(define-reduction (==> --> :=<1>)
  ...
  [(ζ (Stxξ (and (Stx (Lst (? id? id_syntax) _) _ctx) stx) _ξ)
      κ Σ)
   syntax? :=<1> (id=? id_syntax 'syntax Σ)
   #:when syntax?
  ...
  [(ζ (Stxξ (and (Stx (Lst (? id? id_ls)
                           ...
                           stx_body) ctx) stx) ξ)
      κ₀ Σ₀)
   let-syntax? :=<1> (id=? id_ls 'let-syntax Σ₀)
   #:when let-syntax?
